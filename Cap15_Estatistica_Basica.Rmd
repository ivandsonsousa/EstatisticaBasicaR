...
```{r}
result = anova(aov(Y ~ W, data = dados))
result
```
...
```{r}
result = anova(aov(desempenho ~ turno, dados))
result
...
```{r}
result = anova(aov(peso_perdido ~ regime, dados))
result
...
```{r}
result = anova(aov(notas ~ marca, dados))
result
...
```{r}
result = anova(aov(q_agua ~ tipo_laje, dados))
result
...
```{r}
result = anova(aov(q_absorvida ~ processo, dados))
result
...
```{r}
result = anova(aov(nota ~ metodo, dados))
result
...
Como rejeitamos hipótese de igualdade das 3 médias, podemos determinar os intervalos de confiança 2 a 2 para as diferenças entre médias. Nesse caso, teremos 3 combinações possíveis. Vamos começar calculando as médias de cada grupo:
...
Agora, vamos calcular as grandezas necessárias para a a construção dos IC's. Vamos também calcular o quantil para o intervalo de confiança de 95%, considerando o fator de correção de Bonferroni:


```{r}
n = nrow(dados)
I = length(levels(dados$metodo)) #número de grupos
m = 3 #número de comparações
qb = qt(1-(1/m)*0.05/2, df = n - I) #quantil
se = sqrt(result$`Mean Sq`[2])
```


Como todos os grupos possuem o mesmo tamanho ($n=8$), o erro padrão para o estimador da diferença entre médias será:

```{r}
ep = se*sqrt(1/8 + 1/8)
```

Finalmente, vamos determinar os IC's
...
```{r}
result = anova(aov(vendas ~ embalagem, dados))
result
...
Vamos agora construir os intervalos de confiança 2 a 2 para a diferença de médias. Novamente, temos 3 grupos, de modo que existêm 3 comparações possíveis. Vamos iniciar calculando as médias dentro de cada grupo:
...
Agora, vamos calcular as grandezas necessárias para a construção dos IC's:


```{r}
n = nrow(dados) #quantidade total de elementos
m = 3 #Número de comparações
I = length(levels(dados$embalagem))
qb = qt(1-(1/m)*0.05/2, df = n - I) #quantil
se = sqrt(result$`Mean Sq`[2])
```


Como todos os grupos possuem o mesmo tamanho ($n=4$), o erro padrão para o estimador da diferença entre médias será sempre igual a:
...
```{r}
result = anova(aov(nota ~ receita, dados))
result
...
```{r}
result = anova(aov(numero_vezes ~ livro, dados))
result
...
Como rejeitamos a hipótese de igualdade das 4 médias, podemos agora analisar a igualdade duas a duas. Para isso, vamos primeiro calcular e enumerar as comparações possíveis:


```{r}
config = combn(1:4, 2) #comparações possíveis
config
m = ncol(config) #número de comparações possíveis
```

Ou seja, podemos fazer 6 comparações. Vamos primeiro calcular a média de cada grupo:
...
Agora, vamos calcular o desvio padrão dos resíduos e o erro padrão para cada comparação:
...
n3 = grupos$n[3]
n4 = grupos$n[4]
I = length(levels(dados$livro)) #número de grupos
#Devio padrão dos resíduos
se = sqrt(result$`Mean Sq`[2])
#Erro padrão para as diferenças entre médias
...
#Quantil corrigido para o IC(95%)
qb = qt(1-(1/m)*0.05/2, df = nrow(dados) - I)
```

Finalmente, vamos determinar os IC's dois a dois:
...
Agora, vamos calcular o desvio padrão dos resíduos, $S_e^2$, bem como o erro padrão para cada comparação. Devemos notar também que o tamanho dos grupos são diferentes, conforme mostrado acima.


```{r}
#Tamanhos dos grupos
n1 = grupos$n[1] #Tamanho do grupo matinal
n2 = grupos$n[2] #Tamanho do grupo noturno
n3 = grupos$n[3] #Tamanho do grupo da tarde
n = nrow(dados)
I = length(levels(dados$turno))
m = 3 #Número de comparações

#Desvio padrão dos resíduos
se = sqrt(result$`Mean Sq`[2])

#Erro padrão para as comparações entre médias:
ep12 = se*sqrt(1/n1 + 1/n2)
ep13 = se*sqrt(1/n1 + 1/n3)
ep23 = se*sqrt(1/n2 + 1/n3)

#Quantil para o IC(95%), corrigido pelo fator de Bolfarrini:
qb = qt(1-(1/m)*0.05/2, df = n - I)
```
...
cat(
  ...
  grupos$xbar[1] - grupos$xbar[2] + c(-1,1)*qb*ep12,
  "\nIC(mu_T - mu_N; 95%) =",
  grupos$xbar[3] - grupos$xbar[2] + c(-1,1)*qb*ep23
)
```
...
Agora, vamos calcular o desvio padrão dos resíduos, $S_e^2$, bem como o erro padrão para cada comparação. Como os tamanhos dos grupos são todos iguais, o erro padrão para comparação entre médias será sempre o mesmo.
